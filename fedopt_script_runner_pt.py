...
if __name__ == "__main__":
    ...
    [trainloader, valloaders, testloader, _ ], num_channels, num_classes = load_partitioned_datasets(
        num_clients=n_clients, dataset_name=dataset_name, 
    ...
    job = SAGMLFlowJob(
        name=f"pt_{dataset_name}_{model_name}_scatter_and_gather",
        n_clients=n_clients,
        num_rounds=num_rounds,
        initial_model=model,
        key_metric="accuracy",  # Optional: Global model selection metric
    )
    ...
    for i in range(n_clients):
        executor = ScriptRunner(
            script=train_script, 
            script_args=f"--num_rounds {num_rounds} --num_clients {n_clients} --dataset_name {dataset_name} --model_name {model_name} --aggregation_function {aggregation_function}"
        )
        job.to(executor, f"site-{i + 1}")

    # Ensure client setup (if needed)
    for i in range(n_clients):
        job.set_up_client(f"site-{i + 1}")

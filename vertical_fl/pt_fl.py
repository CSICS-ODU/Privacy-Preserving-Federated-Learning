...
from vertical_fl.utilities.models import load_model_defination
from vertical_fl.utilities.datasets import load_partitioned_datasets, get_dataloaders_subset, load_partitioned_datasets_vertical
from vertical_fl.utilities.training_utils import save_model, wandb_init,  print_info, get_device, train, test, load_model as load_saved_weights
from vertical_fl.simple_network import SimpleNetwork
...
def main():
    ...
    trainloaders, valloaders, testloaders, num_channels, num_classes = load_partitioned_datasets_vertical(
        num_clients=n_clients, dataset_name=dataset_name, data_path=data_path, batch_size=32
    )
    ...
    client_idx = int(client_name.split("-")[-1]) - 1  # Adjust based on your naming convention
    ...
    while flare.is_running():
        ...
        evaluate(evaluation_model=f"{dataset_name}_{n_clients}_{aggregation_function}_net", device=device, dataset_name=dataset_name, model_name=model_name, n_clients = n_clients , data_path= data_path, client_idx=client_idx)
...
def evaluate(evaluation_model, device, dataset_name, model_name, n_clients, data_path, client_idx):
    ...
    try:
        ...
        test_loader = testloaders[client_idx]    # Use the corresponding test loader
        val_loader = valloaders[client_idx]

        test_loader_size = len(test_loader.dataset)
        print(f"Test loader size: {test_loader_size}")

        train_loader = get_dataloaders_subset(train_loader, test_loader_size)
        print(f"TrainLoader: {train_loader}")
        print(f"TrainLoader Size: {len(train_loader)}")
        print(f"TestLoader: {test_loader}")
        print(f"TestLoader Size: {len(test_loader)}")

...
class EfficientNet(nn.Module):
    def __init__(self, num_channels=1, num_classes=10):  # Default to 1 channel for VFL
        super(EfficientNet, self).__init__()
        # Load EfficientNet-B0 from NVIDIA hub
        self.efficientnet = torch.hub.load("NVIDIA/DeepLearningExamples:torchhub", "nvidia_efficientnet_b0", pretrained=False)
        
        # Modify the first convolutional layer for custom num_channels
        original_conv = self.efficientnet.stem.conv  # Access the first Conv2d layer
        new_conv = nn.Conv2d(
            in_channels=num_channels,  # 1 for R, G, or B
            out_channels=original_conv.out_channels,  # Typically 32 for EfficientNet-B0
            kernel_size=original_conv.kernel_size,
            stride=original_conv.stride,
            padding=original_conv.padding,
            bias=original_conv.bias is not None
        )
        self.efficientnet.stem.conv = new_conv
        
        # Modify the classifier layer for num_classes

...
from vertical_fl.utilities.lib import blockPrinting
from vertical_fl.utilities.cifar100_fine_coarse_labels import remapping
import pdb,traceback
from typing import List
...
class ChannelSplitDataset(Dataset):
    """Custom Dataset to extract a specific RGB channel from CIFAR-10 images."""
    def __init__(self, dataset, channel_idx):
        self.dataset = dataset
        self.channel_idx = channel_idx  # 0 for R, 1 for G, 2 for B
    ...
    def __getitem__(self, idx):
        ...
        # Extract the specified channel (image is a tensor from transform)
        channel_image = image[self.channel_idx:self.channel_idx+1, :, :]  # Shape: (1, 32, 32)
        return channel_image, label
...
def load_partitioned_datasets_vertical(num_clients=3, dataset_name='CIFAR10', data_path="~/dataset", batch_size=32):
    """Load CIFAR-10 and split it into RGB channels for vertical FL."""
    if num_clients != 3:
        raise ValueError("Vertical FL with RGB split requires exactly 3 clients.")

    dataset = DatasetWrapper(dataset_name, data_path)
    
    # Create datasets for each RGB channel
    trainsets = [ChannelSplitDataset(dataset.trainset, i) for i in range(3)]  # R, G, B
    testsets = [ChannelSplitDataset(dataset.testset, i) for i in range(3)]   # R, G, B

    # Create DataLoaders
    trainloaders = [DataLoader(trainset, batch_size=batch_size, shuffle=True) for trainset in trainsets]
    testloaders = [DataLoader(testset, batch_size=batch_size) for testset in testsets]
    valloaders = [None] * num_clients  # No validation split for simplicity; add if needed

    num_channels = 1  # Each client gets 1 channel
    num_classes = dataset.num_classes

    return trainloaders, valloaders, testloaders, num_channels, num_classes